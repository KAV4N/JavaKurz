...
final public class PouzivatelskeRozhranieDatabazyKnih extends DatabazaKnih{
    ...
    private boolean nacitajDatabazu(){
        boolean uspech;
        if(uspech = nacitajStavDatabazy()){
            System.out.println("Databáza kníh bola úspešne načítaná!");
        } else{
            System.out.println("Nepodarilo sa načítať databázu kníh");
        }
        return uspech;
    }


    private boolean ulozDazabazu(){
        boolean uspech;
        if(uspech = ulozStavDatabazy()){
            System.out.println("Stav databázy kníh bol úspešne ukončený!");
        }else{
            System.out.println("!!!NEPODARILO sa uložiť aktuálny stav databázy!!!");
        }
        return uspech;
    }
    private boolean otazkaUlozDatabazu(){
        boolean uspech = true;
        System.out.println("Prajete si uložiť zmeny v načítanej databáze? (y/n)");
        String vstup = scanner.nextLine();
        if (vstup.equals("y")){
            uspech = ulozDazabazu();
        }
        else {
            System.out.println("Zmeny NEBOLI uložené!");
        }
        return uspech;
    }
    private boolean otazkaVytvorPDF(){
        boolean uspech = true;
        System.out.println("Vytvorit PDF súbor s aktuálnej knižnice kníh? (y/s)");
        String vstup = scanner.nextLine();
        if (vstup.equals("y")){
            uspech = vytvorPDF();
        }
    ...
    private void testVytvorPDF() {
        if(otazkaVytvorPDF()){
            System.out.println("PDF súbor bol úspešne vytovorený");
        }
        else{
            System.out.println("!!!NEPODARILO sa uložiť knižnicu do PDF súboru!!!");
        }

    }
    private void testOtvorPDF(){
        if (!otvorPDF()){
            System.out.println("!!!Súbor PDF sa NEPODARILO otvoriť!!!");
        }
    }
...
//        naplnKniznicu();
    ...
    private void zobrazMenu() {
        ...
        7. Ulož databázu kníh
        8. Načítaj databázu kníh
        9. Uloženie databázy kníh do PDF súboru
        10. otvorenie PDF súboru
        x0x. Vymazať všetky knihy
    ...
    private void ukonciProgram() {
        ...
        if (!zalohaZoznamKnih.equals(zoznamKnih)){
            if (!otazkaUlozDatabazu()){
                status = 1;
            }
        }
        System.exit(status);
